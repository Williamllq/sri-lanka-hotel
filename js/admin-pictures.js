...
/**
 * 将管理员界面的图片保存到前端界面
 * Save admin picture to frontend site storage
 */
function savePictureToSite(adminPicture) {
    try {
        ...
        if (existingIndex !== -1) {
            ...
            console.log(`Updated picture ${adminPicture.name} in site storage`);
        } else {
            sitePictures.push(sitePicture);
            console.log(`Added picture ${adminPicture.name} to site storage`);
        }
        ...
        console.error('Error saving picture to site storage:', e);
        return false;
    }
}
