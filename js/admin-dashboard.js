...
function initTransportSettings() {
    ...
    function saveTransportSettings() {
        ...
        try {
            ...
            const rushHourInput = document.getElementById('rushHourMultiplier');
            const nightInput = document.getElementById('nightMultiplier');
            const weekendInput = document.getElementById('weekendMultiplier');
            const sedanInput = document.getElementById('sedanRate');
            const suvInput = document.getElementById('suvRate');
            const vanInput = document.getElementById('vanRate');
            const luxuryInput = document.getElementById('luxuryRate');
            
            // 检查DOM元素是否存在
            if (!baseFareInput || !ratePerKmInput || !rushHourInput || !nightInput || 
                !weekendInput || !sedanInput || !suvInput || !vanInput || !luxuryInput) {
                console.error('一个或多个表单元素未找到：', {
                    baseFare: !!baseFareInput,
                    ratePerKm: !!ratePerKmInput,
                    rushHour: !!rushHourInput,
                    night: !!nightInput,
                ...
                throw new Error('表单元素未找到，请刷新页面重试');
            }
            
            // 获取表单值
            const baseFareValue = parseFloat(baseFareInput.value);
            const ratePerKmValue = parseFloat(ratePerKmInput.value);
            const rushHourValue = parseFloat(rushHourInput.value);
            const nightValue = parseFloat(nightInput.value);
            const weekendValue = parseFloat(weekendInput.value);
            const sedanValue = parseFloat(sedanInput.value);
            const suvValue = parseFloat(suvInput.value);
            const vanValue = parseFloat(vanInput.value);
            const luxuryValue = parseFloat(luxuryInput.value);
            ...
            if (isNaN(baseFareValue) || isNaN(ratePerKmValue) || isNaN(rushHourValue) ||
                ...
                console.error('无效的数值输入：', {
                    baseFare: isNaN(baseFareValue),
                    ratePerKm: isNaN(ratePerKmValue),
                    rushHour: isNaN(rushHourValue),
                    night: isNaN(nightValue),
                    weekend: isNaN(weekendValue),
                    sedan: isNaN(sedanValue),
                    suv: isNaN(suvValue),
                    van: isNaN(vanValue),
                    luxury: isNaN(luxuryValue)
                });
                
                throw new Error('请在所有字段中输入有效的数字');
            }
            ...
            const transportSettings = {
                ...
                version: '1.1' // 添加版本号以检测是否为新的数据格式
            };
            
            console.log('保存设置对象:', transportSettings);
            ...
            try {
                ...
                console.warn('清除旧设置失败:', e);
            }
            
            // 保存到localStorage - 使用多种方法以确保成功
            try {
                // 方法1：直接设置
                const settingsJSON = JSON.stringify(transportSettings);
                console.log('设置JSON:', settingsJSON);
                
                window.localStorage.setItem('transportSettings', settingsJSON);
                console.log('方法1保存成功');
                
                // 方法2：使用全局变量窗口属性
                window.savedTransportSettings = transportSettings;
                console.log('方法2保存成功 (window属性)');
                
                // 方法3：使用sessionStorage作为备份
                window.sessionStorage.setItem('transportSettings', settingsJSON);
                console.log('方法3保存成功 (sessionStorage)');
                
                // 验证保存结果
                const savedData = window.localStorage.getItem('transportSettings');
                console.log('验证：已保存的数据:', savedData);
                
                if (savedData) {
                    console.log('保存成功 - 数据验证通过');
                    alert('Transport settings saved successfully!');
                    
                    // 触发自定义事件通知其他组件设置已更新
                    const event = new CustomEvent('transportSettingsUpdated', { 
                        detail: transportSettings 
                    });
                    ...
                    console.error('无法验证保存的设置');
                    alert('Settings might not have saved properly. Please try again and check console for details.');
                }
            } catch (e) {
                ...
                try {
                    // 使用window属性作为备选
                    window._transportSettings = transportSettings;
                    console.log('使用备用方法保存成功');
                    alert('Saved using fallback method. Please refresh to see changes.');
                } catch (fallbackError) {
    ...
    function loadTransportSettings() {
        console.log('开始加载transport settings...');
        ...
        try {
            ...
            // 来源3：window全局变量
            const windowSettings = window.savedTransportSettings || window._transportSettings;
            
            // 来源4：备用全局变量
            const backupSettings = window._transportSettings;
            
            // 选择可用的设置（按优先级顺序）
            let settings = localStorageSettings || sessionStorageSettings || windowSettings || backupSettings;
            
            console.log('找到的设置：', {
                localStorage: !!localStorageSettings,
                sessionStorage: !!sessionStorageSettings,
                windowProperty: !!windowSettings,
                backupProperty: !!backupSettings,
                final: !!settings
            });
            
            // 如果没有找到任何设置，使用默认值
            if (!settings) {
                console.log('未找到任何设置，使用默认值');
                settings = defaultSettings;
                
                // 保存默认值到localStorage以便将来使用
                try {
                    window.localStorage.setItem('transportSettings', JSON.stringify(defaultSettings));
            ...
            console.log('Transport settings加载成功');
            ...
            setTimeout(() => {
                console.log('当前表单值：', {
                    ...
                    rushHour: rushHourMultiplier.value,
                    night: nightMultiplier.value,
                    weekend: weekendMultiplier.value,
                    sedan: sedanRate.value,
                    suv: suvRate.value,
                    van: vanRate.value,
                    luxury: luxuryRate.value
                });
            }, 500);
        } catch (error) {
            console.error('加载transport settings时出错：', error);
        }
        
        // 辅助函数：从localStorage获取设置
        function getSettingsFromLocalStorage() {
            try {
                const settingsStr = window.localStorage.getItem('transportSettings');
                console.log('从localStorage读取的原始设置：', settingsStr);
                
                if (!settingsStr) return null;
                
                return JSON.parse(settingsStr);
            } catch (e) {
                console.error('从localStorage解析设置时出错：', e);
                return null;
            }
        }
        
        // 辅助函数：从sessionStorage获取设置
        function getSettingsFromSessionStorage() {
            try {
                const settingsStr = window.sessionStorage.getItem('transportSettings');
                console.log('从sessionStorage读取的原始设置：', settingsStr);
                
                if (!settingsStr) return null;
                
                return JSON.parse(settingsStr);
            } catch (e) {
                console.error('从sessionStorage解析设置时出错：', e);
                return null;
            }
        }
    }
    ...
    if (transportSection) {
        ...
        if (saveBtn && saveBtn.parentNode) {
            saveBtn.parentNode.appendChild(debugBtn);
    ...
    // 调试函数 - 显示当前存储的信息
    function debugTransportSettings() {
        console.log('-------- Transport Settings Debug Info --------');
        ...
        // 检查 localStorage
        try {
            const localData = window.localStorage.getItem('transportSettings');
            debugInfo.localStorage = {
                exists: !!localData,
                raw: localData,
                parsed: localData ? JSON.parse(localData) : null
            };
        } catch (e) {
        ...
        // 检查 sessionStorage
        try {
            const sessionData = window.sessionStorage.getItem('transportSettings');
            debugInfo.sessionStorage = {
                exists: !!sessionData,
                raw: sessionData,
                parsed: sessionData ? JSON.parse(sessionData) : null
            };
        } catch (e) {
            debugInfo.sessionStorage = { error: e.message };
        }
        
        // 检查表单值
        try {
            const formElements = [
                'baseFare', 'ratePerKm', 'rushHourMultiplier', 
                'nightMultiplier', 'weekendMultiplier', 
                'sedanRate', 'suvRate', 'vanRate', 'luxuryRate'
            ];
            
            formElements.forEach(id => {
                const element = document.getElementById(id);
                if (element) {
                    debugInfo.formValues[id] = {
                        value: element.value,
                        type: element.type,
                        isVisible: element.offsetParent !== null
                    };
                } else {
                    debugInfo.formValues[id] = { error: 'Element not found' };
                }
            });
        } catch (e) {
            debugInfo.formValues = { error: e.message };
        }
        
        // 检查全局变量
        debugInfo.windowProps = {
            savedTransportSettings: !!window.savedTransportSettings,
            _transportSettings: !!window._transportSettings
        };
        ...
        // 创建文本表示
        let debugText = 'TRANSPORT SETTINGS DEBUG INFO\n\n';
        
        // 添加localStorage信息
        debugText += '== localStorage ==\n';
        if (debugInfo.localStorage.exists) {
            debugText += '✓ Settings exist in localStorage\n';
            const data = debugInfo.localStorage.parsed;
            if (data) {
                debugText += `baseFare: ${data.baseFare}\n`;
                debugText += `ratePerKm: ${data.ratePerKm}\n`;
                // 添加其他关键属性...
            }
        } else {
            debugText += '✗ No settings in localStorage\n';
        }
        
        // 添加sessionStorage信息
        debugText += '\n== sessionStorage ==\n';
        if (debugInfo.sessionStorage.exists) {
            debugText += '✓ Settings exist in sessionStorage\n';
        } else {
            debugText += '✗ No settings in sessionStorage\n';
        }
        
        // 添加表单值信息
        debugText += '\n== 当前表单值 ==\n';
        for (const [id, info] of Object.entries(debugInfo.formValues)) {
            if (info.error) {
                debugText += `${id}: ERROR - ${info.error}\n`;
            } else {
                debugText += `${id}: ${info.value} (visible: ${info.isVisible ? 'yes' : 'no'})\n`;
            }
        }
